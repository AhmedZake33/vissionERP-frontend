<template>
  <div class="erp-page">
    <div class="erp-page__heading">
      <div>
        <p class="erp-eyebrow">{{ $t('erp.rawMaterialPayments.eyebrow') }}</p>
        <h2>{{ $t('erp.rawMaterialPayments.title') }}</h2>
        <p class="text-muted mb-0">{{ $t('erp.rawMaterialPayments.subtitle') }}</p>
      </div>
    </div>

    <div class="erp-stat-grid">
      <b-card class="erp-stat-card"><span>{{ $t('erp.stats.paymentTransactions') }}</span><strong>{{ stats.total_count }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.stats.totalPaid') }}</span><strong class="text-success">{{ currency(stats.total_amount) }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.invoices.cash') }}</span><strong>{{ currency(stats.cash_total) }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.invoices.instapay') }}</span><strong>{{ currency(stats.instapay_total) }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.invoices.bankTransfer') }}</span><strong>{{ currency(stats.bank_transfer_total) }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.invoices.cheque') }}</span><strong>{{ currency(stats.cheque_total) }}</strong></b-card>
    </div>
    ...
    <b-card class="erp-panel">
      <div class="erp-toolbar">
        <b-input-group class="erp-search">
          ...
          <b-form-input v-model="filters.search" :placeholder="$t('erp.rawMaterialPayments.searchPlaceholder')" @keyup.enter="applyFilters" />
        </b-input-group>
      ...
      <b-table :items="items" :fields="fields" :busy="loading" responsive hover show-empty>
        ...
        <template #cell(invoice)="data">{{ data.item.invoice ? data.item.invoice.invoice_number : '-' }}</template>
        <template #cell(supplier)="data">{{ data.item.invoice && data.item.invoice.supplier ? data.item.invoice.supplier.name : '-' }}</template>
  ...
  computed: {
    fields() {
      return [
        ...
        { key: 'invoice', label: this.$t('erp.rawMaterialPurchases.invoice') },
        { key: 'supplier', label: this.$t('erp.suppliers.supplier') },
  ...
  methods: {
    ...
    async fetch() {
      ...
      try {
        ...
        const { data } = await erp.rawMaterialPayments.list(params)
