<template>
  <div class="erp-page">
    <div class="erp-page__heading">
      <div>
        <p class="erp-eyebrow">{{ $t('erp.rawMaterialPurchases.eyebrow') }}</p>
        <h2>{{ $t('erp.rawMaterialPurchases.title') }}</h2>
        <p class="text-muted mb-0">{{ $t('erp.rawMaterialPurchases.subtitle') }}</p>
      </div>
      <b-button v-if="can('erp.raw-material-purchases.create')" variant="primary" @click="openCreate">
        <feather-icon icon="PlusIcon" class="mr-50" /> {{ $t('erp.rawMaterialPurchases.create') }}
      </b-button>
    </div>

    <div class="erp-stat-grid">
      <b-card class="erp-stat-card"><span>{{ $t('erp.stats.totalInvoices') }}</span><strong>{{ stats.total_count }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.stats.totalAmount') }}</span><strong>{{ currency(stats.total_amount) }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.invoices.paid') }}</span><strong class="text-success">{{ currency(stats.paid_amount) }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.invoices.remaining') }}</span><strong class="text-danger">{{ currency(stats.remaining_amount) }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.common.paid') }}</span><strong>{{ stats.paid_count }}</strong></b-card>
      <b-card class="erp-stat-card"><span>{{ $t('erp.common.unpaid') }}</span><strong>{{ stats.unpaid_count }}</strong></b-card>
    </div>

    <b-card class="erp-panel">
      <div class="erp-toolbar">
        <b-form-input v-model="filters.search" class="erp-search" :placeholder="$t('erp.rawMaterialPurchases.searchPlaceholder')" @keyup.enter="applyFilters" />
      ...
      <b-table :items="items" :fields="fields" :busy="loading" responsive hover show-empty>
        ...
        <template #cell(actions)="data">
          <div class="erp-actions">
            ...
            <b-button v-if="can('erp.raw-material-purchases.edit')" variant="flat-warning" class="btn-icon" @click="openEdit(data.item)"><feather-icon icon="Edit2Icon" /></b-button>
            <b-button v-if="can('erp.raw-material-purchases.delete')" variant="flat-danger" class="btn-icon" @click="remove(data.item)"><feather-icon icon="Trash2Icon" /></b-button>
          </div>
        </template>
      </b-table>

      <div class="erp-pagination">
        <span>{{ pagination.total }} {{ $t('erp.rawMaterialPurchases.title') }}</span>
        <b-pagination v-model="pagination.current_page" :total-rows="pagination.total" :per-page="pagination.per_page" @change="changePage" />
      </div>
    </b-card>

    <b-modal v-model="formModal" :title="editing ? $t('erp.rawMaterialPurchases.edit') : $t('erp.rawMaterialPurchases.create')" hide-footer size="xl">
      <b-form @submit.prevent="save">
        <b-row>
          <b-col cols="12" md="4">
            <b-form-group :label="$t('erp.suppliers.supplier')"><b-form-select v-model="form.supplier_id" :options="supplierOptions" required /></b-form-group>
          </b-col>
          <b-col cols="12" md="3"><b-form-group :label="$t('erp.invoices.invoiceDate')"><b-form-input v-model="form.invoice_date" type="date" required /></b-form-group></b-col>
          <b-col cols="12" md="3"><b-form-group :label="$t('erp.invoices.dueDate')"><b-form-input v-model="form.due_date" type="date" /></b-form-group></b-col>
        </b-row>

        <div class="erp-section-title mt-1">
          <h4>{{ $t('erp.rawMaterialPurchases.items') }}</h4>
        ...
        <div v-for="(item, index) in form.items" :key="index" class="invoice-line">
          <b-row align-v="end">
            <b-col cols="12" md="4"><b-form-group :label="$t('erp.rawMaterials.material')"><b-form-select v-model="item.raw_material_id" :options="materialOptions" required @change="selectMaterial(item)" /></b-form-group></b-col>
            <b-col cols="6" md="2"><b-form-group :label="$t('erp.common.quantity')"><b-form-input v-model.number="item.quantity" type="number" min="0.001" step="0.001" required /></b-form-group></b-col>
            <b-col cols="6" md="2"><b-form-group :label="$t('erp.rawMaterialPurchases.unitCost')"><b-form-input v-model.number="item.unit_cost" type="number" min="0" step="0.01" /></b-form-group></b-col>
            ...
            <b-col cols="3" md="1" class="pb-1 text-right"><b-button variant="flat-danger" class="btn-icon" :disabled="form.items.length === 1" @click="removeItem(index)"><feather-icon icon="XIcon" /></b-button></b-col>
          </b-row>
        </div>
    ...
    <b-modal v-model="detailModal" :title="$t('erp.invoices.details')" ok-only size="lg">
      <div v-if="selected">
        <div class="d-flex justify-content-between mb-2">
          <div><h4>{{ selected.invoice_number }}</h4><span class="text-muted">{{ selected.supplier && selected.supplier.name }}</span></div>
        ...
        <div class="erp-section-title mt-2">
          ...
          <b-button v-if="canPay(selected)" size="sm" variant="outline-success" @click="openPayment(selected)"><feather-icon icon="DollarSignIcon" class="mr-50" /> {{ $t('erp.invoices.pay') }}</b-button>
        </div>
        <b-table :items="selected.payments || []" :fields="paymentFields" small responsive show-empty>
          ...
          <template #cell(actions)="data"><b-button v-if="can('erp.raw-material-payments.delete')" variant="flat-danger" class="btn-icon" @click="deletePayment(data.item)"><feather-icon icon="Trash2Icon" /></b-button></template>
        </b-table>
      </div>
    </b-modal>
    ...
    <b-modal v-model="paymentModal" :title="$t('erp.invoices.payInvoice')" hide-footer>
      <b-form @submit.prevent="savePayment">
        <b-alert v-if="paymentInvoice" show variant="light-info">{{ paymentInvoice.invoice_number }} - {{ $t('erp.invoices.remaining') }}: <strong>{{ currency(paymentInvoice.remaining_amount) }}</strong></b-alert>
        <b-form-group :label="$t('erp.invoices.paymentAmount')"><b-form-input v-model.number="paymentForm.amount" type="number" min="0.01" step="0.01" required /></b-form-group>
        <b-form-group :label="$t('erp.invoices.paymentMethod')"><b-form-select v-model="paymentForm.payment_method" :options="paymentMethods" required /></b-form-group>
        <b-form-group :label="$t('erp.invoices.paymentDate')"><b-form-input v-model="paymentForm.payment_date" type="date" required /></b-form-group>
        <b-form-group :label="$t('erp.invoices.referenceNumber')"><b-form-input v-model="paymentForm.reference_number" /></b-form-group>
        <b-form-group :label="$t('erp.common.notes')"><b-form-textarea v-model="paymentForm.notes" rows="3" /></b-form-group>
        <div class="text-right">
          <b-button variant="flat-secondary" class="mr-1" @click="paymentModal = false">{{ $t('erp.common.cancel') }}</b-button>
          <b-button type="submit" variant="success" :disabled="paying"><b-spinner v-if="paying" small class="mr-50" /> {{ $t('erp.invoices.recordPayment') }}</b-button>
        </div>
      </b-form>
    </b-modal>
  </div>
...
const emptyItem = () => ({ raw_material_id: null, quantity: 1, unit_cost: 0, discount: 0, tax: 0 })
...
export default {
  ...
  computed: {
    fields() {
      return [
        { key: 'invoice_number', label: this.$t('erp.rawMaterialPurchases.invoice') },
    ...
    itemFields() {
      return [
        { key: 'material_name', label: this.$t('erp.rawMaterials.material') },
        { key: 'code', label: this.$t('erp.rawMaterials.code') },
        { key: 'quantity', label: this.$t('erp.common.quantity') },
        { key: 'unit_cost', label: this.$t('erp.rawMaterialPurchases.unitCost') },
        { key: 'total', label: this.$t('erp.customers.total') },
      ]
    },
    paymentFields() {
    ...
    supplierOptions() {
      return [{ value: null, text: this.$t('erp.suppliers.supplier') }, ...this.suppliers.map(item => ({ value: item.id, text: item.name }))]
    },
    materialOptions() {
      return [{ value: null, text: this.$t('erp.rawMaterials.selectMaterial') }, ...this.materials.map(item => ({ value: item.id, text: `${item.name} - ${item.code} (${item.stock_quantity})` }))]
    },
    totals() {
      return this.form.items.reduce((totals, item) => {
        totals.subtotal += Number(item.quantity || 0) * Number(item.unit_cost || 0)
  ...
  methods: {
    ...
    emptyForm() {
      return { supplier_id: null, invoice_date: new Date().toISOString().slice(0, 10), due_date: '', notes: '', items: [emptyItem()] }
    },
    emptyPaymentForm() {
      return { amount: 0, payment_method: 'cash', payment_date: new Date().toISOString().slice(0, 10), reference_number: '', notes: '' }
    },
    canPay(invoice) {
      return this.can('erp.raw-material-payments.create') && Number(invoice?.remaining_amount || 0) > 0
    },
    async fetch() {
      this.loading = true
      try {
        ...
        const { data } = await erp.rawMaterialPurchases.list(params)
    ...
    async fetchOptions() {
      const [suppliers, materials] = await Promise.all([
        erp.suppliers.list({ per_page: 100, status: 'active' }),
        erp.rawMaterials.list({ per_page: 100, status: 'active' }),
      ])
      this.suppliers = suppliers.data.data || suppliers.data
      this.materials = materials.data.data || materials.data
    },
    applyFilters() {
    ...
    async openEdit(item) {
      const { data } = await erp.rawMaterialPurchases.get(item.id)
      this.editing = true
      this.editId = item.id
      this.form = {
    ...
    selectMaterial(item) {
      const material = this.materials.find(row => row.id === item.raw_material_id)
      if (material) item.unit_cost = Number(material.cost)
    },
    lineTotal(item) {
      return Math.max(Number(item.quantity || 0) * Number(item.unit_cost || 0) - Number(item.discount || 0) + Number(item.tax || 0), 0)
    },
    async save() {
      this.saving = true
      try {
        if (this.editing) await erp.rawMaterialPurchases.update(this.editId, this.form)
        else await erp.rawMaterialPurchases.create(this.form)
    ...
    async savePayment() {
      ...
      this.paying = true
      try {
        await erp.rawMaterialPurchases.pay(this.paymentInvoice.id, this.paymentForm)
        this.paymentModal = false
        this.notify(this.$t('erp.invoices.paymentSaved'), 'success')
        await this.fetch()
        if (this.selected && this.selected.id === this.paymentInvoice.id) {
          const { data } = await erp.rawMaterialPurchases.get(this.paymentInvoice.id)
          this.selected = data
        }
      } catch (error) {
        const errors = error.response?.data?.errors
    ...
    async deletePayment(payment) {
      ...
      await erp.rawMaterialPurchases.deletePayment(this.selected.id, payment.id)
      this.notify(this.$t('erp.invoices.paymentDeleted'), 'success')
      await this.fetch()
      const { data } = await erp.rawMaterialPurchases.get(this.selected.id)
      this.selected = data
    },
    async remove(item) {
      const answer = await this.$swal({ title: this.$t('erp.invoices.deleteTitle'), text: this.$t('erp.rawMaterialPurchases.deleteText'), icon: 'warning', showCancelButton: true, confirmButtonText: this.$t('erp.common.deleteConfirm') })
      if (!answer.isConfirmed && !answer.value) return
      await erp.rawMaterialPurchases.remove(item.id)
